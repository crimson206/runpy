...
class TestRunpySchema:
    ...
    @allure.title(
        "Given registered functions, When calling schema, Then returns API-style docs"
    )
    ...
    @allure.title(
        "Given functions with shortcuts, When generating schema, Then includes shortcut info"
    )
    def test_schema_with_shortcuts(self):
        ...
        with allure.step("When I register with shortcuts and get schema"):
            ...
            cli = Runpy()
            cli.register(
                deploy, shortcuts={"name": "n", "version": "v", "environment": "e"}
            )
    ...
    @allure.title(
        "Given nested command structure, When requesting schema, Then shows full hierarchy"
    )
    ...
    @allure.title(
        "Given schema request with format, When generating, Then returns requested format"
    )
    ...
    @allure.title(
        "Given functions with complex types, When generating schema, Then handles properly"
    )
    def test_schema_complex_types(self):
        with allure.step("Given functions with various type hints"):
            ...
            class ProcessConfig(BaseModel):
                ...
                retry_count: int = Field(
                    3, ge=0, le=10, description="Number of retries"
                )
                mode: Literal["fast", "normal", "thorough"] = Field(
                    "normal", description="Processing mode"
                )
        ...
        with allure.step("Then complex types are properly documented"):
            ...
            # List type
            assert (
                params["items"]["type"] == "array" or params["items"]["type"] == "list"
            )

            # BaseModel type
            assert (
                params["config"]["type"] == "string"
                or params["config"]["type"] == "object"
            )
