...
class TestRunpyAdvanced:
    ...
    @allure.title("Given FastAPI-like docstrings, When parsing, Then extracts parameter descriptions")
    ...
    @allure.title("Given functions with return values, When executed, Then displays formatted output")
    ...
    @allure.title("Given function that raises exceptions, When error occurs, Then shows friendly message")
    ...
    @allure.title("Given config file with defaults, When running commands, Then uses config values")
    def test_configuration_file_support(self):
        ...
        with allure.step("When I create CLI with config"):
            def deploy(service: str, environment: str = "dev", timeout: int = 10) -> str:
                """Deploy a service"""
        ...
        with allure.step("Then config defaults are used"):
            ...
            result = runner.invoke(cli.app, ['deploy', '--service', 'web', '-e', 'staging'])
            assert result.exit_code == 0
