...
class TestRunpyBasicConversion:
    ...
    @allure.title("Given a simple function, When registered, Then it becomes a CLI command")
    ...
    @allure.title("Given a function with defaults, When called without all args, Then uses defaults")
    ...
    @allure.title("Given a function with *args, When called with multiple values, Then processes all")
    ...
    @allure.title("Given shortcuts config, When calling with shortcuts, Then maps correctly")
    ...
    @allure.title("Given typed parameters, When called with wrong types, Then shows error")
    def test_type_validation(self):
        ...
        with allure.step("When I call with invalid types"):
            ...
            result = runner.invoke(cli.app, [
                'calculate', '--x', 'not_a_number', '--y', '2.5'
            ])
