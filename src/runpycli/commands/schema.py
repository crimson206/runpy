...
def collect_commands(group: click.Group, schema: dict, runpy_instance: "Runpy", path: str = "") -> None:
    ...
    for cmd_name, cmd in group.commands.items():
        ...
        if isinstance(cmd, click.Group):
            ...
            collect_commands(cmd, schema, runpy_instance, f"{path}/{cmd_name}" if path else cmd_name)
        else:
            # It's a command
...
def get_command_schema(cmd: click.Command, cmd_name: str, path: str, runpy_instance: "Runpy") -> dict:
    ...
    for param in cmd.params:
        ...
        if isinstance(param, click.Option):
            ...
            if func_info:
                for p in func_info["parameters"]:
                    if p["name"] == param_name:
                        param_schema["type"] = get_schema_type_from_annotation(p["annotation"])
                        break
                else:
            ...
            if shortcut_key in runpy_instance.shortcuts and param_name in runpy_instance.shortcuts[shortcut_key]:
                param_schema["shortcut"] = runpy_instance.shortcuts[shortcut_key][param_name]
